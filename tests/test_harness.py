...
from defect_sense.eval.harness import (
    ImageRecord, evaluate_category, summarize, write_outputs,
)


def make_record(true_type, verdict, pred_type=None, type_correct=None,
                triaged=True, s1=0.1, s2=2.0):
    return ImageRecord(
        path="x.png", true_type=true_type, verdict=verdict, anomaly_score=0.5,
        triaged_to_vlm=triaged, pred_type=pred_type, type_correct=type_correct,
        stage1_seconds=s1, stage2_seconds=s2 if triaged else 0.0,
    )
...
def test_evaluate_category_walks_folders(tmp_path):
    ...
    class FakePipeline:
        def inspect(self, image):
            ...
            return InspectionResult(verdict="pass", anomaly_score=0.1, triaged_to_vlm=False)
    ...
    limited = evaluate_category(FakePipeline(), tmp_path, "hazelnut", limit=1, progress=False)
...
def test_write_outputs(tmp_path):
    ...
    csv_path, json_path = write_outputs(records, summary, tmp_path)
